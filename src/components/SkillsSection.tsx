      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid gap-6 md:gap-8 md:grid-cols-2" data-animate>
          {Object.entries(skills).map(([category, { icon, items }], index) => (
            <Card 
              key={category} 
              className="glass hover:bg-purple-500/10 transition-all duration-500 hover:scale-[1.03] group overflow-hidden relative"
              data-stagger
            >
              <div className="absolute top-0 left-0 w-2 h-full bg-gradient-to-b from-purple-500 via-pink-500 to-blue-500 opacity-60 group-hover:opacity-100 transition-all duration-500 group-hover:w-3"></div>
              ...
              <CardContent className="flex flex-wrap gap-3 p-5 md:p-6 pt-0 relative z-10">
                {items.map((skill, index) => (
                  <Badge 
                    key={index} 
                    variant="secondary"
                    className="bg-white/10 hover:bg-white/20 transition-all duration-300 transform hover:scale-110 hover:shadow-glow cursor-default py-2 px-4 text-sm font-medium text-foreground"
                  >
                    {skill}
                  </Badge>
                ))}
